...
import { deleteBatchPermanently, getDeletedBatches, restoreBatch } from '../../../api/services/products';
...
const DeletedBatchCard: React.FC<BatchCardProps> = ({ productId }) => {
    ...
    const handleDelete = async (id: number) => {
        setIsLoading(true);
        try {
            const res = await deleteBatchPermanently(id);
            if (res.status === 200) {
                setBatches((prev) => prev.filter((item) => item.id !== id));
                Toast('success', res.data.message || 'Batch deleted permanently successfully');
            }
        } catch (err) {
    ...
    const handleGetDeletedBatches = async (page = 1, perPage = 10, search = '') => {
        setIsLoading(true);
        try {
            const res = await getDeletedBatches(page, perPage, search, productId as number | String);
            if (res.status === 200) {
    ...
    const handleRestore = async (id: number) => {
        setIsLoading(true);
        try {
            const res = await restoreBatch(id);

            if (res.status === 200) {
                setBatches((prev) => prev.filter((item) => item.id !== id));
                Toast('success', res.data.message || 'Batch restore successfully');
            }
        } catch (err) {
            const axiosError = err as AxiosError<any>;
                            ...
                            <ul className="flex items-center justify-center gap-2">
                                <li>
                                    <Tippy content="Restore">
                                        <button type="button" onClick={() => handleRestore(batch?.id as number)}>
            ...
            {batches.length > 0 && (
                <Pagination
                    ...
                    onPageChange={(newPage) => {
                        ...
                        handleGetDeletedBatches(newPage, pagination.perPage);
                    }}
                />
            )}
...
export default DeletedBatchCard;
